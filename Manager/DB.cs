...
internal class DB
{
    ...
    public static int InsertProducts(string connectionString)
    {
        ...
        while (ans == "y")
        {
            ...
            string query = "INSERT INTO Products([Category_ID], [Product_Name], [Description], [Price], [ImageURL])" +
                "VALUES(@Category_ID,@Product_Name,@Description,@Price,@ImageURL)";
            using (SqlConnection con = new SqlConnection(connectionString))
    ...
    public static int InsertCategories(string connectionString)
    {
        ...
        while (ans == "y")
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                cmd.Parameters.Add("@Category_Name", SqlDbType.VarChar).Value = categoryName;
